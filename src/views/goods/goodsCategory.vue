  ...
  <div class="container">
    ...
    <el-card>
      ...
      >
        <template slot="isstop" scope="scope">
          <i v-if="scope.row.isCancellation==0" class="el-icon-success" />
          <i v-else class="el-icon-error" />
        </template>
         <template slot="ismain" scope="scope">
          <i v-if="scope.row.isMainMaterial==1" class="el-icon-success" />
          <i v-else class="el-icon-error" />
        </template>
    ...
    >
      ...
      >
        ...
        <el-row>
          ...
          <el-col :sm="6" :md="6" :lg="6" :xl="6">
            <el-form-item label="主材" prop="isMainMaterial">
              <el-switch
                v-model="cateData.isMainMaterial"
                :inactive-value="0"
                :active-value="1"
              />
            </el-form-item>
          ...
          <el-col :sm="6" :md="6" :lg="6" :xl="6">
            <el-form-item label="作废" prop="isCancellation">
              <el-switch
                v-model="cateData.isCancellation"
                :inactive-value="0"
                :active-value="1"
              />
            </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="danger" @click="submitGoodsCategory('cateform')"
          >确定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      columns: [
        { label: "类别代码", prop: "categoryId", headerAlign: "center" },
        { label: "类别名称", prop: "categoryName", headerAlign: "center" },
        {
          label: "作废",
          type: "template",
          template: "isstop",
        ...
        { label: "上级代码", prop: "parentId", headerAlign: "center" },
        {
          label: "排序索引",
          prop: "findex",
          width: "80",
  ...
  methods: {
    ...
    showDialog(isAdd, row) {
      ...
      if (isAdd) {
        ...
        this.cateData.isMainMaterial = 0;
        ...
        this.cateData.isMainMaterial = row.isMainMaterial;
        ...
        .then((res) => {
          if (res.code == 200 && res.returnStatus == 1) {
            if (res.result.status) {
              ...
              return this.$message.warning(res.result.errorMsg);
            }
          } else {
            this.$message.error(res.msg);
          }
